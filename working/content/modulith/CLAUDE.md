...
CI runs `dotnet ef migrations has-pending-model-changes` for every `DbContext` the host registers, so a model change that lands without its migration fails the build. No database is needed for that check — it compares the current model against the last migration's snapshot.
